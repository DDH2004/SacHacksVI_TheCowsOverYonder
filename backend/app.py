...
def advance_day():
    global gameState
    global exponent
    global goal_interval
    
    # Apply the effects of the next day's news (generated on the previous day)
    if "nextDayNews" in gameState:
        news = gameState["nextDayNews"]
    ...
    # Generate news for the next day
    next_day_news = generate_news_events(gameState["companies"])
    for news_item in next_day_news:
        news_item["timestamp"] = "Today"
    gameState["nextDayNews"] = next_day_news
